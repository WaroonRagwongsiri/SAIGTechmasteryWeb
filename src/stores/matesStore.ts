...
export const useMatesStore = create<MatesStore>((set, get) => ({
  ...
  fetchMates: async (query = '', reset = true) => {
    ...
    try {
      const res = await fetch(`/api/mates?query=${encodeURIComponent(query)}&page=${page}&limit=${state.limit}`, { 
        credentials: 'include' 
      });
  ...
  loadMoreMates: async () => {
    ...
    set({ loading: true, error: null });
    try {
      const res = await fetch(`/api/mates?query=${encodeURIComponent(state.searchQuery)}&page=${page}&limit=${state.limit}`, { 
        credentials: 'include' 
      });
